...
public class SensAppHelper {
    ...
    private static ContentValues buildComposite(Context context, String name, String description) throws IllegalArgumentException {
        if (context == null) {
            throw new IllegalArgumentException("The context is null");
        } else if (name == null) {
            throw new IllegalArgumentException("The composite name is null");
        } else if (isSensorRegistered(context, name)) {
            return null;
        }
        ContentValues values = new ContentValues();
        ...
        if (description == null) {
            values.put(SensAppContract.Composite.DESCRIPTION, "");
        } else {
    ...
    public static Uri registerComposite(Context context, String name, String description) throws IllegalArgumentException {
        ContentValues values = buildComposite(context, name, description);
        if (values == null) {
            return null;
        }
        return context.getContentResolver().insert(SensAppContract.Composite.CONTENT_URI, values);
    }

    public static boolean renameComposite(Context context, String name, String newName) {
        ContentValues values = new ContentValues();
        values.put(SensAppContract.Composite.NAME, newName);
        context.getContentResolver().update(Uri.parse(SensAppContract.Composite.CONTENT_URI + "/" + name), values, null, null);
        values.clear();
        values.put(SensAppContract.Compose.COMPOSITE, newName);
        context.getContentResolver().update(Uri.parse(SensAppContract.Compose.CONTENT_URI + "/" + name), values, null, null);
        return true;
    }

    public static boolean isCompositeRegistered(Context context, String name) {
        Cursor c = context.getContentResolver().query(Uri.parse(SensAppContract.Composite.CONTENT_URI + "/" + name), new String[]{SensAppContract.Composite.NAME}, null, null, null);
        boolean isRegistered = false;
        if (c != null) {
            isRegistered = c.getCount() > 0;
            c.close();
    ...
    private static ContentValues buildCompose(Context context, String name, String composite, String sensor) throws IllegalArgumentException {
        if (context == null) {
            throw new IllegalArgumentException("The context is null");
        } else if (name == null) {
            throw new IllegalArgumentException("The compose name is null");
        ...
        ContentValues values = new ContentValues();
        //values.put(SensAppContract.Compose.ID, name);
        values.put(SensAppContract.Compose.COMPOSITE, composite);
        values.put(SensAppContract.Compose.SENSOR, sensor);

        return values;
    }

    public static Uri registerCompose(Context context, String name, String composite, String sensor) throws IllegalArgumentException {
        ContentValues values = buildCompose(context, name, composite, sensor);
        if (values == null) {
            return null;
        }
        return context.getContentResolver().insert(SensAppContract.Compose.CONTENT_URI, values);
    }

    public static boolean renameCompose(Context context, String name, String newName) {
        ContentValues values = new ContentValues();
        values.put(SensAppContract.Compose.ID, newName);
        context.getContentResolver().update(Uri.parse(SensAppContract.Compose.CONTENT_URI + "/" + name), values, null, null);
        return true;
    }

    public static boolean isComposeRegistered(Context context, String composite, String sensor) {
        String where = SensAppContract.Compose.SENSOR + " = \"" + sensor + "\" AND " + SensAppContract.Compose.COMPOSITE + " = \"" + composite + "\"";
        Cursor c = context.getContentResolver().query(Uri.parse(SensAppContract.Compose.CONTENT_URI + "/"), new String[]{SensAppContract.Compose.SENSOR}, where, null, null);
        boolean isRegistered = false;
        if (c != null) {
            isRegistered = c.getCount() > 0;
            c.close();
